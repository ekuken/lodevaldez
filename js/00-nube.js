...
/* ============================================================
   COLA DE IMPRESIÓN
   ------------------------------------------------------------
   Para imprimir desde una computadora que no tiene la impresora
   enchufada, o desde un celular. El que manda no imprime: deja
   el ticket anotado en la nube. La computadora que tiene la
   impresora lo levanta y lo saca por su impresora de siempre.

   Así no hace falta compartir nada en la red del café —que es la
   misma que usan los clientes— ni andar con contraseñas de
   Windows dando vueltas entre computadoras.
   ============================================================ */

/* Qué hace ESTA computadora con los tickets. Es por equipo, no por café:
   se guarda en el navegador y no viaja a la nube.
     local     → los imprime en su propia impresora (lo de siempre)
     cola      → los manda a la impresora del café
     impresora → es la que tiene la impresora: imprime lo suyo y lo que le
                 mandan los demás                                          */
function modoImpresionKey(){ return 'cafe_impresion_v1_' + LOCAL; }
function modoImpresion(){
  try{ return localStorage.getItem(modoImpresionKey()) || 'local'; }
  catch(e){ return 'local'; }
}
function setModoImpresion(m){
  try{ localStorage.setItem(modoImpresionKey(), m); }catch(e){}
  if (m === 'impresora') nubeMirarImpresiones();
}
/* Un identificador de este equipo, para saber quién imprimió qué */
function nubeEquipoId(){
  try{
    let id = localStorage.getItem('cafe_equipo_v1');
    if (!id){ id = 'eq-' + Math.random().toString(36).slice(2, 8); localStorage.setItem('cafe_equipo_v1', id); }
    return id;
  }catch(e){ return 'eq-?'; }
}

/* ---------- Mandar ---------- */
async function nubeMandarAImprimir(titulo, html, ancho){
  if (!nubeIniciar() || !NUBE.activa || !LOCAL)
    return { ok: false, msg: 'Sin conexión con la nube: no se puede mandar a la impresora del café' };
  try{
    const { error } = await NUBE.cli.from('impresiones').insert({
      local_id: LOCAL, titulo: titulo || '', html: html, ancho: ancho || 80,
      pedido_por: (typeof USUARIO !== 'undefined' && USUARIO) ? USUARIO.nombre : ''
    });
    if (error){ nubeFallo('mandando a la impresora del café', error); return { ok: false, msg: 'No se pudo mandar' }; }
    return { ok: true };
  }catch(e){ nubeFallo('mandando a la impresora del café', e); return { ok: false, msg: 'No se pudo mandar' }; }
}

/* ---------- Recibir e imprimir ----------
   Solo lo hace el equipo marcado como "impresora". Los tickets viejos NO se
   imprimen: si la computadora estuvo apagada dos horas, nadie quiere que al
   prenderla salgan cuarenta papeles de golpe. Se marcan como impresos y se
   descartan.                                                              */
const NUBE_TICKET_VIEJO = 10 * 60 * 1000;   /* más de 10 minutos = viejo */
let NUBE_IMPRIMIENDO = false;
let NUBE_COLA_LIMPIA = false;   /* los ya impresos se borran una vez por sesión */

async function nubeMirarImpresiones(){
  if (modoImpresion() !== 'impresora') return;
  if (NUBE_IMPRIMIENDO || !NUBE.activa || !LOCAL || !nubeIniciar()) return;
  if (document.visibilityState !== 'visible') return;
  NUBE_IMPRIMIENDO = true;
  if (!NUBE_COLA_LIMPIA){ NUBE_COLA_LIMPIA = true; nubeLimpiarImpresiones(); }
  try{
    const { data, error } = await NUBE.cli.from('impresiones')
      .select('id, titulo, html, creado')
      .eq('local_id', LOCAL).is('impreso', null)
      .order('creado', { ascending: true }).limit(5);
    if (error){ nubeFallo('mirando la cola de impresión', error); NUBE_IMPRIMIENDO = false; return; }
    for (const t of (data || [])){
      const viejo = (Date.now() - msDeIso(t.creado)) > NUBE_TICKET_VIEJO;
      /* Se reclama el ticket ANTES de imprimirlo: si hay dos computadoras
         marcadas como impresora en el mismo café, solo una se lo lleva y no
         sale por duplicado. La condición "impreso is null" hace de candado. */
      const r = await NUBE.cli.from('impresiones')
        .update({ impreso: new Date().toISOString(), impreso_por: nubeEquipoId() })
        .eq('id', t.id).is('impreso', null).select('id');
      if (r.error || !r.data || !r.data.length) continue;   /* se lo llevó otra */
      if (viejo){
        console.warn('[nube] ticket "' + t.titulo + '" descartado por viejo (más de 10 minutos).');
        continue;
      }
      nubeImprimirRecibido(t);
      break;   /* de a uno: window.print() bloquea hasta que termina */
    }
  }catch(e){ /* chequeo de fondo */ }
  NUBE_IMPRIMIENDO = false;
}
function msDeIso(s){ const t = s ? new Date(s).getTime() : NaN; return isNaN(t) ? 0 : t; }

function nubeImprimirRecibido(t){
  const el = document.getElementById('tk');
  if (!el) return;
  el.innerHTML = t.html;
  if (typeof tkAplicarAncho === 'function') tkAplicarAncho();
  if (typeof toast === 'function') toast('🖨 Imprimiendo: ' + (t.titulo || 'ticket'));
  window.print();
}

/* La computadora con la impresora mira la cola seguido: un ticket que sale
   medio minuto tarde ya no sirve en la cocina. */
setInterval(nubeMirarImpresiones, 5000);

/* Los tickets ya impresos no hacen falta para nada: se borran los de más de
   un día para que la tabla no crezca sin control. */
async function nubeLimpiarImpresiones(){
  if (!nubeIniciar() || !NUBE.activa || !LOCAL) return;
  const ayer = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
  try{ await NUBE.cli.from('impresiones').delete().eq('local_id', LOCAL).lt('creado', ayer); }
  catch(e){}
}
